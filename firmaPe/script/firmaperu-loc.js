//
// FirmaPeru 2025 - Optimizado por Mi
// ----------------------------------
//
// Requiere jQuery v3.6.0 (https://code.jquery.com/jquery-3.6.0.min.js)
// Creamos el objeto localmente en lugar de incluirlo en el html
//
//
var jqFirmaPeru = jQuery.noConflict( true );
console.clear();
console.log( "Servicio FirmaPeru 2025 (optimizado)" );
console.log( "====================================" );
...
function startSignature(e, a)
{
    if ("undefined" == typeof jqFirmaPeru) {
        ...
        if (console.log("Firma Per\xfa: La versi\xf3n de JQuery para Firma Per\xfa al momento de su puesta en producci\xf3n es 3.6.0, si usa una versi\xf3n superior y no presenta inconvenientes no ser\xeda necesario bajar la versi\xf3n."),
            ...
            app = clickonce,
            console.log("Firma Per\xfa: Sistema Operativo Windows, se realizar\xe1 la ejecuci\xf3n con ClickOnce.");

        else if ("macOS" === OSName)
            app = jws,
            console.log("Firma Per\xfa: Sistema Operativo macOS, se realizar\xe1 la ejecuci\xf3n con OpenWebStart.");
            ...
            if (/Firefox/.test(userAgent))
                app = jws,
                    console.log("Firma Per\xfa: Sistema Operativo Linux, se realizar\xe1 la ejecuci\xf3n con OpenWebStart.");
            else {
                alert("Firma Per\xfa\n\nNavegador no soportado, solo disponible con Mozilla Firefox.");
                return
            }
            */
            console.log( "ModoLinux FirmaPeru" );
            alert("Debug: Modo Linux " + app);
        ...
        signatureInit(),
            ...
            jqFirmaPeru.ajax({
                ...
                console.log("SERVER CARGADO: "+host);
                console.log("Recuerde: Modificar permisos de escritura en /destino");
                "ok" === e.status && (clearInterval(inter),
                                      processSignature(host, a))
            }).fail(function() {
                0 === count && (printInfo(host, e),
                                runService(e),
                    ...
                    count == maximumTry && (clearInterval(inter),
...
{
    "Windows" === (OSName = getOSName()) ? (console.log("***CONFIGURACI\xd3N PARA SISTEMA OPERATIVO WINDOWS***"),
                                            ...
                                            console.log("Si luego de tener un plugin instalado y no se ejecuta el servicio, aseg\xfarese que la PC tiene acceso al puerto " + a + " que el desarrollador estableci\xf3 para la ejecuci\xf3n del servicio local y que la URL " + e + " no est\xe9 bloqueada por alg\xfan programa local como antivirus o firewall."),
                                            console.log("Puede revisar el log de ejecuci\xf3n en USER_HOME/PCM/FirmadorClienteWeb/log/FirmadorClienteWeb.log"),
                                            console.log("\n")) : "macOS" === OSName ? (console.log("***CONFIGURACI\xd3N PARA SISTEMA OPERATIVO MACOS***"),
                                                                                       ...
                                                                                       console.log("Si luego de realizar las instalaciones y configuraciones el comoponente web no carga, aseg\xfarese que la PC tiene acceso al puerto " + a + " que el desarrollador estableci\xf3 para la ejecuci\xf3n del servicio local y que la URL " + e + " no est\xe9 bloqueada por alg\xfan programa local."),
                                                                                       console.log("Puede revisar el log de ejecuci\xf3n en USER_HOME/PCM/FirmadorClienteWeb/log/FirmadorClienteWeb.log")) : "Linux" === OSName && (console.log("***CONFIGURACI\xd3N PARA SISTEMA OPERATIVO LINUX***"),
    console.log("Revisar instalaciones y configuraciones a realizar para que el componente web funcione: https://apps.firmaperu.gob.pe/web/firmador.xhtml"),
    console.log("Si luego de realizar las instalaciones y configuraciones el comoponente web no carga, aseg\xfarese que la PC tiene acceso al puerto " + a + " que el desarrollador estableci\xf3 para la ejecuci\xf3n del servicio local y que la URL " + e + " no est\xe9 bloqueada por alg\xfan programa local."),
    console.log("Puede revisar el log de ejecuci\xf3n en USER_HOME/PCM/FirmadorClienteWeb/log/FirmadorClienteWeb.log"))
}
...
function runService(e)
{
    console.log("Ejecutando la aplicaci\xf3n Firma Per\xfa Cliente Web."),
        (objFrame = document.getElementById("componentFirmaPeru")) || ((div = document.createElement("div")).innerHTML = '<iframe id="firmaPeru" style="height:0%;width:0%; border:none;display:none;" src="' + app + "?port=" + e + '"></iframe>',
